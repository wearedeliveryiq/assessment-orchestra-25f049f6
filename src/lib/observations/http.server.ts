...
/** Shared REST envelope for the observation APIs. */
export async function handleObservationRoute(
  request: Request,
  ...
  try {
    ...
    if (error instanceof service.ObservationServiceError) {
      return json({ error: error.message }, error.status);
    }
    console.error("[observation-api]", error);
    return json({ error: "Observation engine error" }, 500);
  }
}
...
export async function readJsonBody<T>(request: Request): Promise<T> {
  try {
    ...
    throw new service.ObservationServiceError("Invalid JSON body", 400);
  }
}
