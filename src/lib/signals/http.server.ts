...
/** Shared REST envelope for the signal APIs, mirroring the observation APIs. */
export async function handleSignalRoute(
  ...
  try {
    ...
    if (error instanceof service.SignalServiceError) {
      return json({ error: error.message }, error.status);
    }
    console.error("[signal-api]", error);
    return json({ error: "Signal engine error" }, 500);
  }
}
...
export async function readJsonBody<T>(request: Request): Promise<T> {
  try {
    ...
    throw new service.SignalServiceError("Invalid JSON body", 400);
  }
}
