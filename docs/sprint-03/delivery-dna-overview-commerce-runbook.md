...
3. Verify new Snapshot sessions pin collection/provenance 2.1.0 and presentation policy 2.1.1; only `create_delivery_dna_snapshot_v21` and `link_delivery_dna_snapshot_v21` remain executable by `service_role`; exact 15-ID linking is tenant scoped; historical 1.x and 2.0 rows are unchanged; and client roles hold no new table/function privileges. Completed 2.1 results may be reprojected through presentation policy 2.1.1 without updating stored responses or domain means.
