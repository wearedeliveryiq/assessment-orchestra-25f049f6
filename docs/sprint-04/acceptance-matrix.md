...
| Tenant isolation/permissions       | scoped repositories, authenticated permission boundary, DB membership and deny-by-default ACL         | PASS — live RLS and ACL verified              |
...
| Lovable Cloud migration execution  | managed `20260803121246`, `20260803121309`, plus Cloud-default-grant correction `20260803121414`      | PASS                                          |
...
| Tenant isolation/permissions    | scoped repositories, authenticated permissions, DB active-member assignment and deny-by-default ACL   | PASS — live RLS and ACL verified              |
...
| Lovable Cloud migration         | managed `20260803124204`, `20260803124304`, plus Cloud-default-grant correction `20260803124407`      | PASS                                          |
...
| AC2 authorised typed measures                      | versioned direction, unit, scale, baseline, target, tolerance, target deadline/time zone, cadence, source and owner; active tenant membership checked in API and DB | direction/boundary, configuration, retired, unauthorised actor and cross-tenant fixtures                                                              | PASS — live schema and ACL verified    |
...
| AC4 immutable observations and corrections         | append-only observations and status events; same-scope, acyclic, single-branch superseding corrections; deterministic tie order                                     | `out_supersede_*`, `out_equal_effective_order`, immutability and correction tests                                                                     | PASS — five live triggers verified     |
...
| Tenant/security/privacy      | authenticated permissions, exact scope validation, RLS zero-policy and least-privilege migration assertions                                                    | PASS — live RLS/ACL/functions verified         |
...
| Lovable Cloud migration      | managed `20260803154536`, `20260803154612`, `20260803154643`, plus Cloud-default-grant correction `20260803154747`; live schema/RLS/ACL/type/task verification | PASS                                           |
...
| Tenant isolation/permissions    | scoped repositories, authenticated request context, DB active-member checks and deny-by-default ACL   | PASS — live RLS and ACL verified              |
...
| Lovable Cloud migration         | managed `20260803130952`, `20260803131037`, plus Cloud-default-grant correction `20260803131124`      | PASS                                          |
...
| Live report smoke               | requires first genuine eligible production Delivery DNA portfolio                                       | RECORDED LIMITATION — genuine evidence unavailable |
...
| AC4 proportionate rollback/recovery control passes     | governed rollback and failure controls; PB-004A/PDR-004-001 v1.1 capability assessment, runbook requirements, ownership and explicit residual-risk acceptance                                                                | catalogue rollback, feature replay, partial failure, retry, expiry and controlled recovery tabletop                           | PASS WITH RECORDED LIMITATION — no restore performed or claimed |
...
| AC6 security/performance/resilience/release gates pass | RLS zero-policy, client privilege revocation, scoped permission, rate/batch/attempt/lease/expiry bounds, one-minute task and amended recovery governance                                                                     | security source, 10,000-event performance, 10,001 rejection, idempotency, production build and recovery policy reconciliation | PASS WITH RECORDED LIMITATIONS                                  |
...
| Recovery                        | rollback evidence plus PB-004A/PDR-004-001 v1.1 platform assessment, current-control runbook requirements, ownership and residual-risk acceptance; no restore represented | PASS WITH RECORDED LIMITATION — daily in-place recovery accepted; no fixed RPO/RTO or rehearsal claim |
...
| S4-010 complete locked behaviour and golden fixtures           | PASS — deployed; live schema, ACL and task verified                                                  |
...
| Accessibility, responsive, privacy, performance and resilience | PASS WITH RECORDED LIMITATIONS — full customer journey pending; proportionate recovery risk accepted |
| Approved platform recovery policy                              | PASS — PB-004A/PDR-004-001 v1.1; no fixed RPO/RTO and no restore claim                               |
| Genuine eligible Delivery DNA end-to-end smoke                 | PENDING genuine customer evidence                                                                    |
| Product Owner and Matt Prust Sprint acceptance                 | PASS — SAR-004 v1.2 ACCEPTED WITH RECORDED LIMITATIONS                                               |
