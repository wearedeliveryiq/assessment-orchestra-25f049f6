-- Delivery DNA 2.1 clean cutover. Historical 1.x and 2.0 records remain
-- immutable and are never translated, recalculated or granted 2.1 access.
...
ALTER TABLE public.delivery_dna_snapshot_sessions
  ...
  ADD CONSTRAINT delivery_dna_snapshot_configuration_version
    CHECK (configuration_version IN ('1.0.0', '1.1.0', '2.0.0', '2.1.0')),
  ADD CONSTRAINT delivery_dna_snapshot_presentation_policy_version
    CHECK (presentation_policy_version IN ('1.0.0', '1.1.0', '2.0.0', '2.1.0'));
...
ALTER TABLE public.delivery_dna_snapshot_responses
  ...
  ADD CONSTRAINT delivery_dna_snapshot_responses_question_id_check CHECK (
    question_id ~ '^ddna\.[a-z_]+\.p$'
    OR question_id ~ '^ddna2\.[a-z_]+\.(p|snapshot)$'
  ),
  ADD CONSTRAINT delivery_dna_snapshot_exact_manifest CHECK (
    ...
    OR (question_id = 'ddna2.strategic_alignment.snapshot' AND capability_id = 'strategic_alignment' AND capability_order = 1)
    OR (question_id = 'ddna2.portfolio_prioritisation.snapshot' AND capability_id = 'portfolio_prioritisation' AND capability_order = 2)
    OR (question_id = 'ddna2.benefits_value.snapshot' AND capability_id = 'benefits_value' AND capability_order = 3)
    OR (question_id = 'ddna2.sponsorship_accountability.snapshot' AND capability_id = 'sponsorship_accountability' AND capability_order = 4)
    OR (question_id = 'ddna2.governance_decision_making.snapshot' AND capability_id = 'governance_decision_making' AND capability_order = 5)
    OR (question_id = 'ddna2.risk_assurance_resilience.snapshot' AND capability_id = 'risk_assurance_resilience' AND capability_order = 6)
    OR (question_id = 'ddna2.delivery_approach_lifecycle.snapshot' AND capability_id = 'delivery_approach_lifecycle' AND capability_order = 7)
    OR (question_id = 'ddna2.planning_control_dependencies.snapshot' AND capability_id = 'planning_control_dependencies' AND capability_order = 8)
    OR (question_id = 'ddna2.capacity_delivery_ecosystem.snapshot' AND capability_id = 'capacity_delivery_ecosystem' AND capability_order = 9)
    OR (question_id = 'ddna2.leadership_culture_collaboration.snapshot' AND capability_id = 'leadership_culture_collaboration' AND capability_order = 10)
    OR (question_id = 'ddna2.stakeholder_change_adoption.snapshot' AND capability_id = 'stakeholder_change_adoption' AND capability_order = 11)
    OR (question_id = 'ddna2.delivery_capability_enablement.snapshot' AND capability_id = 'delivery_capability_enablement' AND capability_order = 12)
    OR (question_id = 'ddna2.data_reporting_decision_insight.snapshot' AND capability_id = 'data_reporting_decision_insight' AND capability_order = 13)
    OR (question_id = 'ddna2.learning_adaptability_improvement.snapshot' AND capability_id = 'learning_adaptability_improvement' AND capability_order = 14)
    OR (question_id = 'ddna2.digital_automation_responsible_ai.snapshot' AND capability_id = 'digital_automation_responsible_ai' AND capability_order = 15)
  );
...
ALTER TABLE public.assessment_responses
  ...
  ADD CONSTRAINT assessment_response_snapshot_provenance CHECK (
    ...
    OR (
      ...
      AND provenance_version IN ('1.0.0', '1.1.0', '2.0.0', '2.1.0')
...
BEGIN
  ...
  END IF;
  IF OLD.configuration_version IN ('2.0.0', '2.1.0')
     AND (
...
BEGIN
  ...
  INSERT INTO public.delivery_dna_snapshot_sessions (
    ...
    p_token_hash, '2.1.0', '2.1.0', p_scope_type, trim(p_scope_display_name)
  ) RETURNING id INTO v_id;
  RETURN v_id;
END;
$$;

CREATE OR REPLACE FUNCTION public.link_delivery_dna_snapshot_v21(
  p_token_hash text,
  p_user_id uuid,
...
BEGIN
  ...
  IF p_manifest_metadata #>> '{deliveryDna,assessmentType}' <> 'delivery-dna'
     ...
     OR p_manifest_metadata #>> '{deliveryDna,knowledgePackVersion}' <> '2.1.0'
  ...
  IF v_snapshot.id IS NULL OR v_snapshot.configuration_version <> '2.1.0'
  ...
  SELECT
    ...
    'delivery-dna-snapshot', '2.1.0', responded_at
  FROM public.delivery_dna_snapshot_responses
  WHERE snapshot_session_id = v_snapshot.id
...
-- Disable new 1.x and 2.0 collection/linking without deleting their history.
...
COMMENT ON FUNCTION public.create_delivery_dna_snapshot_v21(text, text, text, text) IS
  'Creates only Delivery DNA Snapshot 2.1 sessions. Historical 1.x and 2.0 sessions are never upgraded.';